--//  === VOIDOR Z SCRIPTHUB V1.3.2 ===
--// ৻ Thank you for using Voidor Z ScriptHub. ৲
print([[
__   _____ ___ ___  ___  ____    ____
\ \ / / _ \_ _|   \/ _ \|  _ \  |_  /
 \ V / (_) | || |) | (_) | |_) |  / / 
  \_/ \___/___|___/ \___/|_.__/  /___|
       S C R I P T H U B  v1.3.2
]])
...
-- ═══ SAVE ════════════════════════════════════════════════
local SAVE_FILE = "voidorz_132.json"
local saveData  = {}
...
local LIGHT = {
    bg=Color3.fromRGB(238,235,255),  top=Color3.fromRGB(220,215,250),
    card=Color3.fromRGB(252,250,255),text=Color3.fromRGB(25,18,55),
    sub=Color3.fromRGB(90,80,130),   off=Color3.fromRGB(185,180,215),
    red=Color3.fromRGB(200,45,45),   bgT=0.04,
    grad0=Color3.fromRGB(220,210,255),grad1=Color3.fromRGB(238,235,255),
    grad2=Color3.fromRGB(230,228,255),loader=Color3.fromRGB(240,237,255),
    starC=Color3.fromRGB(120,90,200),titleC=Color3.fromRGB(70,40,180),
    subC=Color3.fromRGB(100,80,160),
}
...
lTitle.Size=UDim2.new(0,480,0,50)
lTitle.Position=UDim2.new(0.5,-240,0.5,40)
lTitle.BackgroundTransparency=1
lTitle.Font=Enum.Font.GothamBold
lTitle.TextSize=34
lTitle.TextColor3=th0.titleC
lTitle.Text="VOIDOR Z SCRIPTHUB"
lTitle.TextTransparency=1

local lSub=Instance.new("TextLabel",lb)
lSub.Size=UDim2.new(0,400,0,24)
lSub.Position=UDim2.new(0.5,-200,0.5,96)
lSub.BackgroundTransparency=1
lSub.Font=Enum.Font.Gotham
lSub.TextSize=14
lSub.TextColor3=th0.subC
lSub.Text="v1.3.2  |  ScriptHub"
lSub.TextTransparency=1

local lBarTrack=Instance.new("Frame",lb)
lBarTrack.Size=UDim2.new(0,320,0,4)
lBarTrack.Position=UDim2.new(0.5,-160,0.5,136)
lBarTrack.BackgroundColor3=isDark and Color3.fromRGB(30,20,60) or Color3.fromRGB(200,195,235)
...
lStat.Size=UDim2.new(0,320,0,20)
lStat.Position=UDim2.new(0.5,-160,0.5,148)
lStat.BackgroundTransparency=1
lStat.Font=Enum.Font.Gotham
lStat.TextSize=11
lStat.TextColor3=th0.subC
lStat.Text="Initializing..."
lStat.TextTransparency=1

-- ═══ GUI ═════════════════════════════════════════════════
local gui=Instance.new("ScreenGui")
gui.Name="VOIDORZ_132"
gui.ResetOnSpawn=false
gui.IgnoreGuiInset=true
gui.Parent=player.PlayerGui

-- Mobile: use scale-based sizing; Desktop: fixed pixel
local WIN_W,WIN_H = 830,695
local main=Instance.new("Frame",gui)
if IS_MOBILE then
    main.Size=UDim2.new(0.97,0,0.88,0)
    main.Position=UDim2.new(0.015,0,0.06,0)
else
...
do
    ...
    if sc~=100 then
        local s=Instance.new("UIScale",main); s.Scale=sc/100
    end
end
...
top.BackgroundColor3=TH().top; top.BackgroundTransparency=0.15
...
subLbl.Position=UDim2.new(0,51,0,topH-18); subLbl.Size=UDim2.new(0.75,0,0,14)
...
local function updateSubLbl()
    subLbl.Text="v1.3.2  |  "..kbName(KB.hideUI).."=hide  |  "..kbName(KB.fly).."=fly  |  "..kbName(KB.tracers).."=tracers"
end
updateSubLbl()
...
minBtn.MouseButton1Click:Connect(function()
    ...
    local expH = IS_MOBILE and UDim2.new(0.97,0,0.88,0) or UDim2.new(0,WIN_W,0,WIN_H)
...
UIS.InputChanged:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch then doDrag(i.Position) end
end)
...
tabBar.Position=UDim2.new(0,10,0,topH+8); tabBar.Size=UDim2.new(1,-20,0,38)
tabBar.CanvasSize=UDim2.new(0,2400,0,0); tabBar.BackgroundTransparency=1
tabBar.ScrollBarThickness=0; tabBar.BorderSizePixel=0; tabBar.ScrollingDirection=Enum.ScrollingDirection.X
local tabLL=Instance.new("UIListLayout",tabBar)
tabLL.FillDirection=Enum.FillDirection.Horizontal; tabLL.Padding=UDim.new(0,5)

local CONTENT_Y = topH+8+38+4
...
local function createPage(name)
    ...
    sf.ScrollingEnabled=true; sf.ClipsDescendants=true; sf.Active=true
    sf.ElasticBehavior=Enum.ElasticBehavior.WhenScrollable
    sf.Visible=false; tA(sf,"ScrollBarImageColor3")
...
local TAB_W = IS_MOBILE and 95 or 112
local function createTab(name)
    ...
    btn.MouseEnter:Connect(function()
        if not pages[name] or not pages[name].Visible then TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=C.accent2,BackgroundTransparency=0.4}):Play() end
    end)
    btn.MouseLeave:Connect(function()
        if not pages[name] or not pages[name].Visible then TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=TH().card,BackgroundTransparency=0.3}):Play() end
    end)
    tabBtns[name]=btn
end
...
local function makeCard(parent,h,order)
    ...
    card.BackgroundColor3=TH().card; card.BackgroundTransparency=0.28
...
local function secLabel(parent,text,order)
    ...
    line.BackgroundColor3=C.accent; line.BackgroundTransparency=0.65; line.BorderSizePixel=0; tA(line,"BackgroundColor3")
end
...
local function makeSlider(parent,text,minV,maxV,defV,order,onChange)
    ...
    UIS.InputEnded:Connect(handleInputEnd)
    return setVal
end
...
local function makeResetBtn(parent,order,cb)
    ...
    card.BackgroundColor3=isDark and Color3.fromRGB(10,7,24) or Color3.fromRGB(228,222,250)
...
do
    local patches={
        {ver="1.3.2 (current)",ch={
            "Critical launch fix: script now writes via Python to avoid shell buffer truncation",
            "Mobile support: main window uses scale-based UDim2, all inputs handle Touch type",
            "Logo (id: 121589619252959) shown in both the loading screen and topbar",
            "Title correctly reads VOIDOR Z SCRIPTHUB throughout",
            "All tog* variables properly used in keybind handler (were cut off previously)",
            "Fixed applyAccent / applyTheme unused-function warnings",
            "Theme switch repaints in place via tT/tA registries, no GUI rebuild",
            "Scroll fix: AutomaticCanvasSize reset + defer on every tab switch",
        }},
        {ver="1.3.1",ch={
            "Attempted launch fix (file still truncated by bash heredoc buffer)",
            "Theme system rebuilt to repaint in place",
            "Visuals expanded: Neon World, Rainbow Trail, Freeze Character, Darkness",
            "Void Lab: Orbit Platform, Fake Death added",
            "Anti-Fling added to Misc with keybind",
        }},
        {ver="1.3.0",ch={
            "Rebranded to Voidor Z",
            "Dark Mode / Light Mode",
            "Misc tab added",
            "Weapons moved from Void Lab to Visuals",
        }},
        {ver="1.2.0",ch={
            "All keybinds start unbound",
            "Unbind [X] button per keybind",
            "Conflict prevention on bind",
        }},
        {ver="1.1.0 and earlier",ch={
            "Semantic versioning adopted",
            "Full tabbed hub, persistent save, accent theming, loading screen",
            "All base features",
        }},
    }
    ...
    for _,p in ipairs(patches) do
        ...
        vc.BackgroundColor3=isDark and Color3.fromRGB(28,16,62) or Color3.fromRGB(218,210,255)
        vc.BackgroundTransparency=0.1
...
makeABtn(voidPage,"VOID",O(),function() local r=getRoot(); if r then r.CFrame=CFrame.new(r.Position+Vector3.new(0,9e4,0)) end end)
...
local setOrbit=makeToggle(voidPage,"Orbit Platform",O(),function(s)
    if s then
...
end); regT(voidPage,setOrbit,false)
...
makeBtn(voidPage,"Fake Death",Color3.fromRGB(120,40,40),O(),function()
    local hum=getHum(); if not hum then return end
    hum.PlatformStand=true; task.delay(2,function() hum.PlatformStand=false end)
end)
makeBtn(voidPage,"Explode",Color3.fromRGB(180,80,0),O(),function()
...
makeBtn(voidPage,"Restore Body",TH().off,O(),function()
...
-- ═══ VISUALS ══════════════════════════════════════════════
secLabel(visPage,"Weapons & Combat FX",O())
...
local setFreezeChar=makeToggle(visPage,"Freeze Self",O(),function(s)
    local root=getRoot(); if not root then return end
    if s then
        local bg=Instance.new("BodyGyro",root); bg.Name="FreezeGyro"; bg.MaxTorque=Vector3.new(1e9,1e9,1e9); bg.D=500; bg.P=1e6; bg.CFrame=root.CFrame
        local bv=Instance.new("BodyVelocity",root); bv.Name="FreezeVel"; bv.Velocity=Vector3.zero; bv.MaxForce=Vector3.new(1e9,1e9,1e9)
    else
        local r=getRoot(); if not r then return end
        for _,v in ipairs(r:GetChildren()) do if v.Name=="FreezeGyro" or v.Name=="FreezeVel" then v:Destroy() end end
    end
...
makeResetBtn(visPage,O(),function()
    ...
    if root then
        for _,v in ipairs(root:GetChildren()) do
            if v:IsA("Trail") or (v:IsA("Attachment") and v.Name:find("TrailA")) then v:Destroy() end
            if v.Name=="FreezeGyro" or v.Name=="FreezeVel" then v:Destroy() end
        end
    end
...
secLabel(miscPage,"NPCs",O())
makeBtn(miscPage,"Freeze All NPCs",Color3.fromRGB(60,80,160),O(),function()
    for _,m in ipairs(workspace:GetDescendants()) do
        if m:IsA("Humanoid") and m.Parent~=player.Character then m.WalkSpeed=0; m.JumpPower=0 end
    end
end)
makeBtn(miscPage,"Unfreeze All NPCs",TH().off,O(),function()
    for _,m in ipairs(workspace:GetDescendants()) do
        if m:IsA("Humanoid") and m.Parent~=player.Character then m.WalkSpeed=16; m.JumpPower=50 end
    end
end)
makeBtn(miscPage,"Kill All NPCs",Color3.fromRGB(160,40,40),O(),function()
    for _,m in ipairs(workspace:GetDescendants()) do
        if m:IsA("Humanoid") and m.Parent~=player.Character then m.Health=0 end
    end
end)
...
makeBtn(miscPage,"Reset Character",TH().red,O(),function() local h=getHum(); if h then h.Health=0 end end)
...
if not IS_MOBILE then
    ...
    do
        ...
        il.BackgroundTransparency=1; il.Font=Enum.Font.Gotham; il.TextSize=11; il.TextColor3=TH().sub
        il.TextXAlignment=Enum.TextXAlignment.Left; il.TextWrapped=true; tT(il,"TextColor3","sub")
        il.Text="All keybinds start unbound. Click [KEY] to assign, [X] to clear. Saved automatically."
    end
else
    -- Mobile keybind info card
    local ic=makeCard(kbPage,60,O()); ic.BackgroundTransparency=0.3
    local il=Instance.new("TextLabel",ic); il.Size=UDim2.new(1,-20,1,0); il.Position=UDim2.new(0,14,0,0)
    il.BackgroundTransparency=1; il.Font=Enum.Font.Gotham; il.TextSize=13; il.TextColor3=TH().sub
    il.TextXAlignment=Enum.TextXAlignment.Left; il.TextWrapped=true; tT(il,"TextColor3","sub")
    il.Text="Keybinds are desktop only. On mobile, use the toggle switches in each tab directly."
end
...
local function makeThemeCard(parent,order,dark)
    local card=makeCard(parent,68,order)
    card.BackgroundColor3=dark and Color3.fromRGB(12,10,28) or Color3.fromRGB(232,228,255)
    card.BackgroundTransparency=0.08
    local img=Instance.new("ImageLabel",card)
    img.Size=UDim2.new(0,40,0,40); img.Position=UDim2.new(0,12,0.5,-20)
    img.BackgroundTransparency=1; img.Image=LOGO_ID
    local lbl=Instance.new("TextLabel",card); lbl.BackgroundTransparency=1
    lbl.Position=UDim2.new(0,62,0,8); lbl.Size=UDim2.new(0.45,0,0,22)
    lbl.Text=dark and "Dark Mode" or "Light Mode"; lbl.Font=Enum.Font.GothamBold; lbl.TextSize=14
    lbl.TextColor3=dark and Color3.fromRGB(200,185,255) or Color3.fromRGB(60,40,120)
    lbl.TextXAlignment=Enum.TextXAlignment.Left
    local sub2=Instance.new("TextLabel",card); sub2.BackgroundTransparency=1
    sub2.Position=UDim2.new(0,62,0,30); sub2.Size=UDim2.new(0.5,0,0,16)
    sub2.Text=dark and "Dark purple glass" or "Clean light style"; sub2.Font=Enum.Font.Gotham; sub2.TextSize=11
    sub2.TextColor3=dark and Color3.fromRGB(120,110,160) or Color3.fromRGB(100,85,145)
    ...
    applyBtn.BackgroundColor3=dark and Color3.fromRGB(60,40,120) or Color3.fromRGB(130,70,255)
    applyBtn.Text="Apply"; applyBtn.Font=Enum.Font.GothamBold; applyBtn.TextSize=13
    applyBtn.TextColor3=Color3.new(1,1,1); applyBtn.BorderSizePixel=0
    Instance.new("UICorner",applyBtn).CornerRadius=UDim.new(1,0)
    if (dark and isDark) or (not dark and not isDark) then
        local dot=Instance.new("Frame",card)
        dot.Size=UDim2.new(0,8,0,8); dot.Position=UDim2.new(1,-14,0,10)
        dot.BackgroundColor3=C.accent; dot.BorderSizePixel=0
        Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0); tA(dot,"BackgroundColor3")
    end
...
task.spawn(function()
    local tw=TweenService; task.wait(0.15)
    tw:Create(lLogo,  TweenInfo.new(0.6),{ImageTransparency=0}):Play()
    ...
    task.wait(0.5)
    local fade={{lb,"BackgroundTransparency"},{lLogo,"ImageTransparency"},{lTitle,"TextTransparency"},{lSub,"TextTransparency"},{lStat,"TextTransparency"},{lBarTrack,"BackgroundTransparency"},{lBarFill,"BackgroundTransparency"}}
    for _,f in ipairs(fade) do tw:Create(f[1],TweenInfo.new(0.55),{[f[2]]=1}):Play() end
    task.wait(0.65)
    main.Visible=true
    loadGui:Destroy()
end)

print("Voidor Z ScriptHub v1.3.2 loaded | Theme: "..(isDark and "Dark" or "Light"))
